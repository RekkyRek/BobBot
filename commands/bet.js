class give {
  ...
  async handle (message) {
    ...
    if (isNaN(amount)) { message.channel.send(`**${message.author.username}**, you need to specify an amount to bet.`); message.delete(); return }
    ...
    const roll2 = this.r([0, 0, 1, 1, 1, 2])
    ...
    message.channel.send(`**${message.author.username}**, you rolled **${roll1}** and **${roll2}**. You won **${win} ${win === 1 ? 'poem' : 'poems'}**. You now have a total of **${poems} ${poems === 1 ? 'poem' : 'poems'}**`)
    message.delete()
  }
}
